 ...
 * 계보 복원가 호석 골드3
 * Topological Sorting
 * BOJ21276_2_PartialSuccess 개선
 * -> 12/19 부분 성공 1개 case 개선 ㅋㅋ
 */
import java.io.*;
import java.util.*;
public class BOJ21276_3_PartialSuccess {
    public static void main(String[] args) throws IOException{
        ...
        while (!q.isEmpty()){
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int j = 0; j < child[next].size(); j++) {
                    String target = child[next].get(j);
                    if(child[cur].contains(target)){
                        child[next].remove(target);
                    }
                }
            }
        }
